...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Location of data output",
    )
    parser.add_argument(
        "--site-output-dir",
        default=os.path.normpath(os.path.join(SCRIPT_DIR, "../site/public")),
        help="Location of site output",
    )
    parser.add_argument(
        "--python-simulation",
        action="store_const",
...
def main() -> int:
    ...
    for tournament_id, tournament_meta in manifest.items():
        ...
        json_path = os.path.join(args.data_output_dir, f"{tournament_id}.json")
        if args.no_scrape:
        ...
        with open(
            os.path.join(args.data_output_dir, f"ranks/{tournament_id}.json"),
            "w",
            encoding="utf-8",
        ) as f_ranks:
        ...
        embedding_path = os.path.join(
            args.site_output_dir, f"embeddings-{tournament_id}.html"
        )
    ...
    has_any_data = {
        ...
        if os.path.exists(os.path.join(args.data_output_dir, f"{tournament_id}.json"))
    }
    has_embeddings = {
        tournament_id
        for tournament_id in manifest
        if os.path.exists(
            os.path.join(args.site_output_dir, f"embeddings-{tournament_id}.html")
        )
    }
    ...
    index_path = os.path.normpath(os.path.join(args.site_output_dir, "index.html"))
